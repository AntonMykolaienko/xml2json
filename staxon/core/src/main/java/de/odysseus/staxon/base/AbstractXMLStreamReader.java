...
public abstract class AbstractXMLStreamReader<T> implements XMLStreamReader {
    ...
    public AbstractXMLStreamReader(T rootInfo, Location locationProvider) {
        this.scope = new XMLStreamReaderScope<>(XMLConstants.NULL_NS_URI, rootInfo);
    ...
    /**
     * Consume initial event. This method must be called by subclasses prior to any use of an instance (typically in
     * constructor).
     *
    ...
    /**
     * Main method to be implemented by subclasses. This method is called by the reader when the event queue runs dry.
     * Consume some events and delegate to the various <code>readXXX()</code> methods. When encountering an element
     * start event, all attributes and namespace delarations must be consumed too, otherwise these won't be available
     * during start element.
     *
    ...
    /**
     * Read start element. A new scope is created and made the current scope. The provided <code>scopeInfo</code> is
    ...
    protected void readStartElementTag(String prefix, String localName, String namespaceURI, T scopeInfo)
        ...
        scope = new XMLStreamReaderScope<>(scope, prefix, localName, namespaceURI);
    ...
    public void require(int eventType, String namespaceURI, String localName) throws XMLStreamException {
        if (eventType != getEventType()) {
            throw new XMLStreamException("Expected event type " + getEventName(eventType)
                    + ", was " + getEventName(getEventType()), getLocation());
        }
