...
/**
 * Represent document/element scope. Used to store namespace bindings and attributes, implements
        ...
        void verify() throws XMLStreamException {
            if (prefix == null) {
                if (!XMLConstants.NULL_NS_URI.equals(namespaceURI)) {
                    String prfx = AbstractXMLStreamScope.this.getNonEmptyPrefix(namespaceURI);
                    if (prfx == null) {
                ...
                if (!XMLConstants.DEFAULT_NS_PREFIX.equals(prefix)) {
                    String localNamespaceURI = AbstractXMLStreamScope.this.getNamespaceURI(prefix);
                    if (localNamespaceURI == null || XMLConstants.NULL_NS_URI.equals(localNamespaceURI)) {
    ...
    private String findNonEmptyPrefix(String namespaceURI, AbstractXMLStreamScope descendent) {
        ...
        if (isRoot()) {
            if (parent == null) {
                ...
                Iterator<?> prfxs = parent.getPrefixes(namespaceURI);
                while (prfxs.hasNext()) {
                    String prfx = prfxs.next().toString();
                    if (!XMLConstants.DEFAULT_NS_PREFIX.equals(prfx)) {
                        if (descendent == this || descendent.getNamespaceURI(prfx).equals(namespaceURI)) {
                            return prfx;
                        }
                    }
                }
